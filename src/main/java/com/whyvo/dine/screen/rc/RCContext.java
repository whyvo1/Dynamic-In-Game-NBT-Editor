...
public class RCContext {
    ...
    public void render(DrawContext context, int mouseX, int mouseY) {
        ...
        for(RCButton button : this.buttons) {
            int n = font.getWidth(button.getNameText());
            if(n > m) {
                m = n;
            }
        }
        ...
        int y = startY + 5;
        for(RCButton button : this.buttons) {
            context.drawText(font, button.getNameText(), x, y, button.active ? 0xFFFFFFFF : 0xFF555555, false);
            y += bHeight;
        }
    }
    ...
    public static class Builder {
        ...
        public Builder addButton(Text name, Runnable action) {
            this.buttons.add(new RCButton(name, action));
            return this;
        }

        public Builder addButtonInactive(Text name) {
