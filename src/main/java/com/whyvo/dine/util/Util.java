...
public class Util {
    ...
    public static void trySendCommand(String command) {
        ...
        if (oldSendCommand(command, player.connection)) {
            player.displayClientMessage(SUCCESS_COMMAND.apply(command.length()), false);
            if(Config.logSentCommand) {
                DINE.LOGGER.info("Sent command: {}", command);
            }
        }
        else {
            player.displayClientMessage(FAILED_COMMAND, false);
            if(Config.logSentCommand) {
    ...
    private static boolean oldSendCommand(String command, ClientPacketListener networkHandler) {
        SignableCommand<ClientSuggestionProvider> signedArgumentList = SignableCommand.of(networkHandler.getCommands().parse(command, networkHandler.getSuggestionsProvider()));
        if (signedArgumentList.arguments().isEmpty()) {
            networkHandler.send(new ServerboundChatCommandPacket(command));
            return true;
        } else {
            return false;
        }
    ...
    public static void sendErrorCommand() {
        LocalPlayer player = findPlayer();
        if(player == null) return;
        player.displayClientMessage(ERROR_COMMAND, false);
    }
    ...
    public static void copyToClipboard(String string) {
        ...
        LocalPlayer player = findPlayer();
        if(player == null) return;
        player.displayClientMessage(COPIED.apply(string.length()), false);
    }
}
