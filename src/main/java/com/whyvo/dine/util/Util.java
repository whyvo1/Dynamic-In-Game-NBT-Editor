...
public class Util {
    ...
    public static void trySendCommand(String command) {
        ...
        if (oldSendCommand(command, player.networkHandler)) {
    ...
    private static boolean oldSendCommand(String command, ClientPlayNetworkHandler networkHandler) {
        SignedArgumentList<ClientCommandSource> signedArgumentList = SignedArgumentList.of(networkHandler.getCommandDispatcher().parse(command, networkHandler.getCommandSource()));
        if (signedArgumentList.arguments().isEmpty()) {
            networkHandler.sendPacket(new CommandExecutionC2SPacket(command));
            return true;
        } else {
            return false;
        }
